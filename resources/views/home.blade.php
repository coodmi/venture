...
{{-- ═══════════════════════════════════════════════════════════ STATS --}}
@if($stats->count())
<section style="background:#0d0a04;border-bottom:1px solid rgba(212,146,15,.1);padding:3rem 1.5rem;">
    <div style="max-width:80rem;margin:0 auto;display:grid;grid-template-columns:repeat(auto-fit,minmax(120px,1fr));gap:2rem;text-align:center;">
        ...
        <div>
            <div style="font-size:2.25rem;font-weight:800;color:#d4920f;line-height:1;" data-counter="{{ $stat->value }}">{{ $stat->value }}</div>
...
<section style="background:#110e05;padding:5rem 1.5rem;">
    <div style="max-width:80rem;margin:0 auto;">
        ...
        <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(260px,1fr));gap:1.5rem;">
            @php $steps=[['num'=>'01','color'=>'#3b82f6','bg'=>'rgba(59,130,246,.15)','title'=>'Discover','desc'=>'Browse curated investment opportunities filtered by sector, stage, and ticket size.','icon'=>'M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z'],['num'=>'02','color'=>'#10b981','bg'=>'rgba(16,185,129,.15)','title'=>'Connect','desc'=>'Express interest, request meetings, and engage directly with founders and investors.','icon'=>'M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0z'],['num'=>'03','color'=>'#f59e0b','bg'=>'rgba(245,158,11,.15)','title'=>'Grow','desc'=>'Close deals, join bootcamps, attend conferences, and scale your venture together.','icon'=>'M13 7h8m0 0v8m0-8l-8 8-4-4-6 6']]; @endphp
            ...
            <div style="background:#1a1408;border-radius:1.25rem;padding:2rem;border:1px solid rgba(212,146,15,.12);position:relative;overflow:hidden;">
                ...
                <h3 style="font-size:1.125rem;font-weight:700;color:#f0e6c8;margin:0 0 .5rem;">{{ $step['title'] }}</h3>
                <p style="font-size:.875rem;color:#9a8a6a;line-height:1.6;margin:0;">{{ $step['desc'] }}</p>
            </div>
            @endforeach
        </div>
    </div>
...
{{-- ═══════════════════════════════════════════════════════════ HOT DEALS --}}
@if($hotDeals->count())
<section style="background:#0d0a04;padding:4rem 0;">
    <div style="max-width:80rem;margin:0 auto;padding:0 1.5rem;">
        <div style="display:flex;align-items:flex-end;justify-content:space-between;margin-bottom:2rem;flex-wrap:wrap;gap:1rem;">
            <div>
                <span style="display:inline-flex;align-items:center;gap:.375rem;background:rgba(220,38,38,.15);color:#f87171;font-size:.75rem;font-weight:700;padding:.3rem .75rem;border-radius:9999px;margin-bottom:.5rem;">
        ...
        <div id="hotDealsTrack" style="display:flex;gap:1.25rem;overflow-x:auto;scroll-behavior:smooth;padding-bottom:1rem;scrollbar-width:none;-ms-overflow-style:none;" class="hide-scroll">
            ...
            <a href="{{ route('startups.show', $deal->slug) }}" style="text-decoration:none;flex-shrink:0;width:280px;display:block;background:#1a1408;border:1px solid rgba(212,146,15,.15);border-radius:1.25rem;padding:1.5rem;transition:all .25s;position:relative;overflow:hidden;" onmouseover="this.style.boxShadow='0 12px 30px rgba(0,0,0,.4)';this.style.transform='translateY(-3px)';" onmouseout="this.style.boxShadow='none';this.style.transform='translateY(0)';">
                ...
                <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:.875rem;">
                    ...
                    <span style="font-size:.75rem;color:#6b5c3e;font-weight:500;">{{ $deal->sector }}</span>
                </div>
                <h3 style="font-size:1rem;font-weight:700;color:#f0e6c8;margin:0 0 .5rem;line-height:1.4;">{{ $deal->title }}</h3>
                <p style="font-size:.8125rem;color:#9a8a6a;margin:0 0 1.25rem;line-height:1.5;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden;">{{ $deal->solution }}</p>
                <div style="display:flex;align-items:center;justify-content:space-between;padding-top:.875rem;border-top:1px solid rgba(212,146,15,.1);">
                    <span style="font-size:1rem;font-weight:800;color:#d4920f;">৳{{ number_format($deal->ask_amount) }}</span>
                    <span style="font-size:.75rem;color:#d4920f;font-weight:600;">View Details →</span>
                </div>
            </a>
            @endforeach
        </div>
...
<section style="background:#110e05;padding:4rem 0;">
    <div style="max-width:80rem;margin:0 auto;padding:0 1.5rem;">
        <div style="display:flex;align-items:flex-end;justify-content:space-between;margin-bottom:2rem;flex-wrap:wrap;gap:1rem;">
            <div>
            ...
            <a href="{{ route('startups.show', $s->slug) }}" style="text-decoration:none;background:#1a1408;border:1px solid rgba(212,146,15,.15);border-radius:1.25rem;padding:1.5rem;display:flex;flex-direction:column;flex-shrink:0;width:280px;transition:all .25s;overflow:hidden;position:relative;" onmouseover="this.style.boxShadow='0 12px 30px rgba(0,0,0,.4)';this.style.transform='translateY(-3px)';" onmouseout="this.style.boxShadow='none';this.style.transform='translateY(0)';">
                ...
                <div style="display:flex;align-items:flex-start;gap:.75rem;margin-bottom:.875rem;">
                    ...
                    <div style="flex:1;min-width:0;">
                        <h3 style="font-size:.9375rem;font-weight:700;color:#f0e6c8;margin:0 0 .25rem;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">{{ $s->title }}</h3>
                        <div style="display:flex;gap:.3rem;flex-wrap:wrap;">
                            ...
                            @if($s->stage)<span style="font-size:.65rem;background:#1a1408;color:#475569;padding:.15rem .45rem;border-radius:9999px;">{{ $s->stage }}</span>@endif
                            @if($s->is_hot_deal)<span style="font-size:.65rem;background:rgba(220,38,38,.15);color:#f87171;font-weight:700;padding:.15rem .45rem;border-radius:9999px;">🔥</span>@endif
                        </div>
                    </div>
                </div>
                <p style="font-size:.8rem;color:#9a8a6a;line-height:1.55;flex:1;margin:0 0 1rem;display:-webkit-box;-webkit-line-clamp:3;-webkit-box-orient:vertical;overflow:hidden;">{{ $s->business_problem }}</p>
                <div style="display:flex;align-items:center;justify-content:space-between;padding-top:.75rem;border-top:1px solid rgba(212,146,15,.1);">
                    @if($s->ask_amount)<span style="font-size:.9375rem;font-weight:800;color:#d4920f;">৳{{ number_format($s->ask_amount) }}</span>@else<span></span>@endif
                    <span style="font-size:.75rem;color:#d4920f;font-weight:600;">Invest →</span>
                </div>
            </a>
            @endforeach
        </div>
        ...
        <div id="investorTrack" style="display:flex;gap:1.25rem;overflow-x:auto;scroll-behavior:smooth;padding-bottom:1rem;scrollbar-width:none;-ms-overflow-style:none;">
            ...
            <a href="{{ route('investors.show', $inv->id) }}" style="text-decoration:none;background:#1a1408;border:1px solid rgba(212,146,15,.15);border-radius:1.25rem;padding:1.5rem;display:flex;flex-direction:column;flex-shrink:0;width:270px;transition:all .25s;overflow:hidden;position:relative;" onmouseover="this.style.boxShadow='0 12px 30px rgba(0,0,0,.4)';this.style.transform='translateY(-3px)';" onmouseout="this.style.boxShadow='none';this.style.transform='translateY(0)';">
                ...
                <div style="display:flex;align-items:flex-start;gap:.75rem;margin-bottom:.875rem;">
                    ...
                    <div style="flex:1;min-width:0;">
                        <div style="display:flex;align-items:center;gap:.3rem;">
                            <span style="font-size:.9rem;font-weight:700;color:#f0e6c8;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;display:block;">{{ $inv->user->name }}</span>
                        ...
                        <p style="font-size:.72rem;color:#9a8a6a;margin:0;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">{{ $inv->designation }}</p>
                        <p style="font-size:.68rem;color:#6b5c3e;margin:0;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">{{ $inv->organization }}</p>
                    </div>
                </div>
                <div style="display:flex;flex-wrap:wrap;gap:.3rem;margin-bottom:.75rem;">
                    <span style="font-size:.68rem;font-weight:600;padding:.2rem .55rem;border-radius:9999px;{{ $invBadge[$inv->investor_type]??'background:#1a1408;color:#475569;' }}">{{ $invLabels[$inv->investor_type]??$inv->investor_type }}</span>
                    @if($inv->sector_preferences)
                        @foreach(array_slice($inv->sector_preferences,0,2) as $sec)
                        <span style="font-size:.68rem;background:rgba(212,146,15,.15);color:#d4920f;padding:.2rem .5rem;border-radius:9999px;">{{ $sec }}</span>
                        @endforeach
                    @endif
                </div>
                @if($inv->bio)<p style="font-size:.78rem;color:#9a8a6a;line-height:1.5;margin:0 0 .75rem;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden;flex:1;">{{ $inv->bio }}</p>@endif
                ...
                <div style="display:flex;align-items:center;justify-content:space-between;padding-top:.75rem;border-top:1px solid rgba(212,146,15,.1);margin-top:auto;">
                    <span style="font-size:.68rem;color:#6b5c3e;">Ticket</span>
...
<section style="background:#110e05;padding:4rem 0;">
    <div style="max-width:80rem;margin:0 auto;padding:0 1.5rem;">
        <div style="display:flex;align-items:flex-end;justify-content:space-between;margin-bottom:2rem;flex-wrap:wrap;gap:1rem;">
            <div>
                <span style="display:inline-block;background:rgba(5,150,105,.15);color:#34d399;font-size:.75rem;font-weight:700;padding:.3rem .75rem;border-radius:9999px;margin-bottom:.5rem;text-transform:uppercase;letter-spacing:.05em;">Upcoming</span>
        ...
        <div id="eventsTrack" style="display:flex;gap:1.25rem;overflow-x:auto;scroll-behavior:smooth;padding-bottom:1rem;scrollbar-width:none;-ms-overflow-style:none;" class="hide-scroll">
            ...
            <a href="{{ route('events.show', $event->slug) }}" style="text-decoration:none;flex-shrink:0;width:260px;background:#1a1408;border-radius:1.25rem;overflow:hidden;border:1px solid rgba(212,146,15,.12);display:flex;flex-direction:column;transition:all .25s;" onmouseover="this.style.boxShadow='0 12px 30px rgba(0,0,0,.4)';this.style.transform='translateY(-3px)';" onmouseout="this.style.boxShadow='none';this.style.transform='translateY(0)';">
                ...
                <div style="padding:1rem;flex:1;display:flex;flex-direction:column;">
                    <span style="font-size:.7rem;color:#d4920f;font-weight:600;margin-bottom:.25rem;display:block;">{{ $event->start_date->format('M d, Y') }}</span>
                    <h3 style="font-size:.9375rem;font-weight:700;color:#f0e6c8;margin:0 0 .5rem;line-height:1.4;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden;flex:1;">{{ $event->title }}</h3>
                    <p style="font-size:.75rem;color:#6b5c3e;margin:0;">📍 {{ $event->venue ?? 'Online' }}</p>
                </div>
            </a>
            @endforeach
        </div>
...
<section style="background:#0d0a04;padding:4rem 0;">
    <div style="max-width:80rem;margin:0 auto;padding:0 1.5rem;">
        <div style="display:flex;align-items:flex-end;justify-content:space-between;margin-bottom:2rem;flex-wrap:wrap;gap:1rem;">
            <div>
                <span style="display:inline-block;background:rgba(3,105,161,.15);color:#38bdf8;font-size:.75rem;font-weight:700;padding:.3rem .75rem;border-radius:9999px;margin-bottom:.5rem;text-transform:uppercase;letter-spacing:.05em;">📰 Latest</span>
        ...
        <div id="newsTrack" style="display:flex;gap:1.25rem;overflow-x:auto;scroll-behavior:smooth;padding-bottom:1rem;scrollbar-width:none;-ms-overflow-style:none;" class="hide-scroll">
            ...
            <a href="{{ route('news.show', $article->slug) }}" style="text-decoration:none;flex-shrink:0;width:300px;display:flex;flex-direction:column;background:#1a1408;border:1px solid rgba(212,146,15,.15);border-radius:1.25rem;overflow:hidden;transition:all .25s;" onmouseover="this.style.boxShadow='0 12px 30px rgba(0,0,0,.4)';this.style.transform='translateY(-3px)';" onmouseout="this.style.boxShadow='none';this.style.transform='translateY(0)';">
                ...
                <div style="padding:1.125rem;flex:1;display:flex;flex-direction:column;">
                    <span style="font-size:.68rem;color:#d4920f;font-weight:700;text-transform:uppercase;letter-spacing:.06em;">{{ $article->category }}</span>
                    <h3 style="font-size:.9375rem;font-weight:700;color:#f0e6c8;margin:.3rem 0 .5rem;line-height:1.4;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden;flex:1;">{{ $article->title }}</h3>
                    <p style="font-size:.78rem;color:#9a8a6a;line-height:1.5;margin:0 0 .75rem;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden;">{{ $article->summary }}</p>
                    <div style="display:flex;align-items:center;justify-content:space-between;padding-top:.625rem;border-top:1px solid rgba(212,146,15,.1);">
                        <span style="font-size:.72rem;color:#6b5c3e;">{{ $article->published_at?->format('M d, Y') }}</span>
                        <span style="font-size:.72rem;color:#d4920f;font-weight:600;">Read →</span>
                    </div>
                </div>
            </a>
            @endforeach
