...
<section style="background:#f4f7fb;padding:3rem 1.5rem;">
    <div style="max-width:80rem;margin:0 auto;">
        ...
        @else
        @php $sectorColors=['FinTech'=>'#3b82f6','AgriTech'=>'#10b981','HealthTech'=>'#ef4444','EdTech'=>'#f97316','CleanTech'=>'#8b5cf6']; @endphp
        <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(300px,1fr));gap:1.25rem;">
            @foreach($opportunities as $opp)
            @php $sc = $sectorColors[$opp->sector] ?? '#1a3c8f'; @endphp
            <a href="{{ route('startups.show', $opp->slug) }}" style="text-decoration:none;background:#fff;border:1px solid #dde3ea;border-radius:1.25rem;padding:1.5rem;display:flex;flex-direction:column;transition:all .25s;position:relative;overflow:hidden;box-shadow:0 2px 8px rgba(0,0,0,.04);" onmouseover="this.style.boxShadow='0 12px 30px rgba(26,60,143,.12)';this.style.transform='translateY(-3px)';this.style.borderColor='#bfdbfe';" onmouseout="this.style.boxShadow='0 2px 8px rgba(0,0,0,.04)';this.style.transform='translateY(0)';this.style.borderColor='#dde3ea';">
                <div style="position:absolute;top:0;left:0;right:0;height:3px;background:{{ $sc }};"></div>
                <div style="display:flex;align-items:flex-start;justify-content:space-between;margin-bottom:1rem;">
                    <div style="width:3rem;height:3rem;background:{{ $sc }}18;border-radius:.875rem;display:flex;align-items:center;justify-content:center;flex-shrink:0;">
                    ...
                    <div style="display:flex;gap:.375rem;flex-wrap:wrap;justify-content:flex-end;">
                        @if($opp->is_featured)<span style="font-size:.65rem;background:#fff7ed;color:#f97316;font-weight:700;padding:.2rem .5rem;border-radius:9999px;border:1px solid #fed7aa;">⭐ Featured</span>@endif
                        @if($opp->is_hot_deal)<span style="font-size:.65rem;background:#fef2f2;color:#ef4444;font-weight:700;padding:.2rem .5rem;border-radius:9999px;border:1px solid #fecaca;">🔥 Hot</span>@endif
                    </div>
                </div>
                <h3 style="font-weight:700;color:#0d2b6e;font-size:1rem;margin:0 0 .625rem;line-height:1.4;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden;">{{ $opp->title }}</h3>
                ...
                <p style="font-size:.8125rem;color:#8d98a1;line-height:1.55;flex:1;margin:0 0 1rem;display:-webkit-box;-webkit-line-clamp:3;-webkit-box-orient:vertical;overflow:hidden;">{{ $opp->business_problem }}</p>
