<aside id="adminSidebar" style="width:15rem;background:#0d2b6e;border-right:1px solid #1a3c8f;flex-shrink:0;display:flex;flex-direction:column;height:100vh;overflow:hidden;">
    {{-- Logo --}}
    <div style="padding:1.25rem 1.25rem 1rem;border-bottom:1px solid rgba(255,255,255,.1);">
        <a href="{{ route('home') }}" style="display:flex;align-items:center;gap:.5rem;text-decoration:none;">
            ...
            @else
                <div style="width:1.875rem;height:1.875rem;background:#f97316;border-radius:.4rem;display:flex;align-items:center;justify-content:center;flex-shrink:0;">
                    <span style="color:#fff;font-weight:800;font-size:.7rem;">{{ strtoupper(substr($siteName,0,2)) }}</span>
                </div>
                <span style="font-weight:700;font-size:.9375rem;color:#fff;">{{ $siteName }}</span>
            @endif
        </a>
        <p style="font-size:.65rem;color:rgba(255,255,255,.4);margin:.375rem 0 0;text-transform:uppercase;letter-spacing:.1em;font-weight:600;">Admin Panel</p>
    </div>
    ...
    <nav style="flex:1;padding:.75rem;overflow-y:auto;">
        ...
        <a href="{{ route($item['route']) }}"
           style="display:flex;align-items:center;gap:.625rem;padding:.5rem .75rem;border-radius:.625rem;font-size:.8125rem;font-weight:{{ $active?'700':'500' }};text-decoration:none;margin-bottom:.125rem;transition:all .15s;{{ $active?'background:rgba(249,115,22,.2);color:#fb923c;border:1px solid rgba(249,115,22,.3);':'color:rgba(255,255,255,.55);border:1px solid transparent;' }}"
           onmouseover="{{ $active?'':'this.style.background=\'rgba(255,255,255,.08)\';this.style.color=\'#fff\';' }}"
           onmouseout="{{ $active?'':'this.style.background=\'transparent\';this.style.color=\'rgba(255,255,255,.55)\';' }}">
    ...
    {{-- User --}}
    <div style="padding:1rem;border-top:1px solid rgba(255,255,255,.1);">
        <div style="display:flex;align-items:center;gap:.625rem;">
            <div style="width:2rem;height:2rem;background:linear-gradient(135deg,#f97316,#fb923c);border-radius:50%;display:flex;align-items:center;justify-content:center;flex-shrink:0;">
                <span style="color:#fff;font-weight:800;font-size:.75rem;">{{ strtoupper(substr(auth()->user()->name,0,1)) }}</span>
            </div>
            <div style="min-width:0;">
                <p style="font-size:.8125rem;font-weight:600;color:#fff;margin:0;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">{{ auth()->user()->name }}</p>
                <p style="font-size:.7rem;color:rgba(255,255,255,.4);margin:0;">Administrator</p>
            </div>
        </div>
    </div>
</aside>
